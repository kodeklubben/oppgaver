...
def new_level(yml_path):
    ...
    while True:
        try:
            level = int(input("\nOppgave: {}\n  level {}: ".format(yml_path, LEVEL_STR)))
        except ValueError:
            pass
        if MIN_LEVEL <= level and level < MAX_LEVEL:
            return level
        print("Ooops level must be an integer from {} to {}".format(MIN_LEVEL, MAX_LEVEL))


def new_lesson_yml(new_yml_path):
    ''' If a oppgave lacks a lesson.yml this creates that file.

    In detail, it first checks if 'FILE_INFO' contains lines that
    needs to be written into 'lesson.yml'. Examples are 'license' or 'level'
    Otherwise it uses 'new_level()' and makes user write in the level.
    '''
...
def lesson_yml(lesson_yml_path):
    ...
    if Path(lesson_yml_path).is_file():
        ...
        with open(lesson_yml_path_temp, "w") as g:
            for line in update_lesson_yml(yml_data, lesson_yml_path):
                g.write(line)
