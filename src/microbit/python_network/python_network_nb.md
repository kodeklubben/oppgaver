...
Det er mulig å kobble enheter sammen, og sende og motta meldinger fra hverandre. 
Dette kalles for et nettverk. Et nettverk av sammenkobblede nettverk kalles for
...
![Bilde av to micro:bit'er som er kobblet sammen](./network.png)

Det er på dette fundamentet at vi kan bygge alle de andre lagene i nettverket.

Som diagramet viser, en blå og rød micro:bit er kobblet sammen via
krokodilleklemmer. Begge bruker pin 1 for utgang og pin 2 inngang. Utgangen
fra en enhet er kobblet til inngangen på den andre. Det er litt som å vite
...
Det er akkuratt det samme her, du må kobble ledningen riktig!
...
tilbyr deg hans hånd høflig rist den, og tilltat ham som "hans majestet" også
...
datamaskinen) er Morse kode. Den definerer hvordan å sende bokstaver via av og
...
skurdd på fire ganger, men det andre signalet har en lengre varighet (`.-.._`).
...
Vi er endelig på et stadiet hvor vi kan bygge en meldig -- en meling som faktisk
betyr noe for oss mennesker. Dette er det øverste nivået i nettverket. 
...
Morske kode, kan vi kobble til en høytaler som skal spille av tonene som lager
...
# En oppslags tabell av morse kode og tilhørende bokstaver
MORSE_CODE_LOOKUP = {
    ".-": "A",
    "-...": "B",
    "-.-.": "C",
...
while True:
    ...
    while button_a.is_pressed():
        # Spill av en tone - dette er Morse kode vet du ;-)
        music.pitch(880, 10)
    ...
    if key_down_time:
        ...
        if duration < DOT_THRESHOLD:
            ...
            # inkommende Morse koden og vis en prikk på displayet.
            buffer += '.'
            display.show(DOT)
    ...
    # indikere enden av den inkommende Morse koden. Pausen må være lenge enn
