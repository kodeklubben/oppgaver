---
title: Komme igang med MIT App Inventor
level: 1
author: Tjerand Silde
---

# Introduksjon {.intro}
#### Dette er en introduksjon til MIT App Inventor, hvor vi skal lære å lage applikasjoner til Android. Å lage apps i MIT App Inventor er en ganske enkel måte å lage apps på, men vi anbefaler at en er godt kjent med programmering i Scratch før en prøver ut dette, samt kan litt engelsk. Før vi kan lage våre egne apps, må vi sette opp MIT App Inventor og bli litt kjent med hvordan det fungerer.

# Steg 1: Logge inn på MIT App Inventor {.activity}

## Sjekkliste {.check}

+ [Gå til ai2.appinventor.mit.edu](http://ai2.appinventor.mit.edu/).

+ Logg inn med en Google-konto. Dette kan for eksempel være en gmail-adresse. 
  Dersom du lurer på om epost-adressen din er en Google-konto, [kan du sjekke det 
  her](https://support.google.com/accounts/answer/40560?hl=no).

+ Når du har logget inn kommer du til en side hvor MIT App Inventor ber om tillatelse til å åpne kontoen din.
  Dette må du godta, ved å trykke "Tillat", for å kunne bruke MIT App Inventor for å lage applikasjoner.
  
# Steg 2: Starte nytt prosjekt {.activity}

## Sjekkliste {.check}

+ Etter innlogging blir du videresendt til prosjekt-siden til MIT App Inventor. Legg merke til at det er på engelsk, du
  må dermed trykke på "Start new project" for å komme igang med selve programmeringen. Her kan du navngi prosjektet
  akkurat det du ønsker selv, for eksempel "Test". Så blir du videresendt til hvor du programmerer.
...
# Steg 4: Koble til komponenter {.activity}

#### For de som allerede er kjent med programmering i Scratch, så vil dere se en del likheter med hvordan vi programmerer i MIT App Inventor. Både Scratch og MIT App Inventor er "Drag-and-drop", det vil si, vi drar komponenter og kodeblokker fra menyen og til hvor de skal være for å programmere.
 
# Steg 4: Programmere med blokker {.activity}

#### For å programmere komponentene i appen vår så må vi trykke på knappen i høyre hjørne med navnet "Blocks". Da kommer vi til en programmeringsskjerm som ligner veldig mye på Scratch. Ved å trykke på de forkjellige kategoriene (for eksempel "Logic") så får dere opp hvilke funksjoner dere kan bruke. Når dere har valgt funksjon, så kan dere trykke på den og dra den over til kode-feltet. Det samme gjelder for komponentene dere har lagt til i appen.[Trykk her for å se hvordan det ser ut](http://appinventor.mit.edu/explore/designer-blocks.html).

# Steg 5: Lage en test-app {.activity}

#### For å se litt på hvordan dette fungerer, så skal vi lage en test-app. Vi har alleredet opprettet prosjektet og gitt det navn. Da kan vi gå rett på programmeringen. Vi ønsker å lage en app som har en knapp på forsiden hvor det står "Talk to me". Når knappen trykkes så skal appen si "Hello world". Dette kan vi få til ved å gjøre følgende:
...
#### Dersom alle tre punktene er huket av så kan vi gå videre. For å kunne teste appen må vi gjøre følgende:
...
#### Nå ser du at du har en knapp på skjermen din som du kan trykke på. Det vil ikkje skje noe når du trykker på den, rett og slett fordi vi ikke har skrevet noe kode for det enda. Det kommer vi snart til, men aller først skal vi teste litt hva som skjer når du legger til ting samtidig som du er koblet til appen.
...
#### Nå er vi klar for å programmere komponentene i appen vår. Det kan vi gjøre på følgende måte:
...
#### Nå er appen din kodet ferdig, men vi har lyst til å sjekke at den fungerer som den skal.
...
# Steg 10: Lære mer {.activity}

#### For å lære mer om MIT App Inventor så kan det være lurt å sjekke ut følgende lenker:

## Sjekkliste {.check}

+ [MIT App Inventor på Youtube](https://www.youtube.com/results?search_query=app+inventor).
...
+ [Sjekke ut MIT App Inventor sitt bibliotek](http://appinventor.mit.edu/explore/library.html).
