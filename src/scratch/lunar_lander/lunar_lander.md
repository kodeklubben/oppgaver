...
+ Når pil opp-tasten trykkes vil vi at romskipet skal få litt ekstra
  ...
  `for alltid`{.blockcontrol}-løkken din.
    ...
    Vi må bruke `0 - cos av retning`{.blockoperators} i stedet for
    `cos av retning`{.blockoperators} fordi `retning`{.blockmotion} i
...
+ Legg også til en
    ...
    ```blocks
        send melding [Sjekk landing v]
    ```

    rett etter `gjenta til`{.blockcontrol}-løkken.
